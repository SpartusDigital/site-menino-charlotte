...
(() => {
  ...
  const stylesheet =
    ...
    // A slot prefilled through src is showing a real photo: the dashed drop
    // affordance must not print over it (the drop target still works on hover).
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // An explicitly empty placeholder ("") means icon-only: the caption and
      // "or browse files" sub-line don't fit slots smaller than ~150x110
      // (logo wells), where .frame{overflow:hidden} would clip them.
      const phAttr = this.getAttribute('placeholder');
      const iconOnly = phAttr !== null && phAttr.trim() === '';
      this._cap.textContent = iconOnly ? '' : (phAttr || 'Drop an image');
      this._cap.style.display = iconOnly ? 'none' : '';
      this._sub.style.display = iconOnly ? 'none' : '';
